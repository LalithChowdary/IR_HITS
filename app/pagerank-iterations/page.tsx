...
import { api, PageRankResult } from '../lib/api';
import { Loader2, ArrowLeft } from 'lucide-react';
import Link from 'next/link';

export default function PageRankIterationsPage() {
  const [loading, setLoading] = useState(true);
  const [pagerankData, setPagerankData] = useState<PageRankResult | null>(null);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await api.runPageRankWithHistory();
        setPagerankData(data);
      } catch (error) {
        console.error('Error fetching PageRank history:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-zinc-50 via-blue-50 to-purple-50 dark:from-zinc-950 dark:via-zinc-900 dark:to-zinc-950">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        ) : pagerankData && pagerankData.history ? (
          <div className="space-y-8">
            {pagerankData.history.map((iteration) => (
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-zinc-200 dark:divide-zinc-700">
                    ...
                    <tbody className="bg-white dark:bg-zinc-900 divide-y divide-zinc-200 dark:divide-zinc-800">
                      {Object.keys(iteration.scores).map((node) => (
                        <tr key={node}>
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-zinc-900 dark:text-zinc-100">
                            {node}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-zinc-500 dark:text-zinc-400">
                            {iteration.scores[node].toFixed(6)}
                          </td>
                        </tr>
                      ))}
                    </tbody>
